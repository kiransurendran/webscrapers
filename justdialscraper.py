...
def get_tags(body):
	return body.find('span',class_='margin0').text.strip().replace("\t",'').replace("\n",'')


def get_rating(body):
	rating = 0.0
	text = body.find('span',class_='star_m')
	if text is not None:
		for item in text:
...
def get_rating_count(body):
	text = body.find('span',class_='rt_count').string
...
def get_address(body):
	return body.find('span',class_='mrehover').text.strip()


def get_location(body):
	text = body.find('a',class_='rsmap')
	if text == None:
		return
...
while True:
	...
	url="https://www.justdial.com/Mumbai/Dell-Laptop-Repair-Services-in-Kurla/nct-10935353-%s" % (page_number)
